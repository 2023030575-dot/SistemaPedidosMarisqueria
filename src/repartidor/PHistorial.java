...
public class PHistorial extends JPanel {
    ...
    private void loadOrders() {
        ...
        ArrayList<PedidoH> orders = deliver.getDoneOrders(deliver.getDeliverId(Main.getInstance().getId()));

        if (orders.isEmpty()) {
            ordersListPanel.add(new JLabel("No hay pedidos."));
        } else {
            for (PedidoH order : orders) {
    ...
    private void searchDoneOrders() {
        ...
        ArrayList<PedidoH> filteredOrders = deliver.searchDoneOrders(searchTerm, deliver.getDeliverId(Main.getInstance().getId()));
        if (filteredOrders.isEmpty()) {
            ordersListPanel.add(new JLabel("No se encontraron pedidos que coincidan."));
        } else {
            for (PedidoH order : filteredOrders) {
                ordersListPanel.add(createOrderRow(order));
            }
        }
    ...
    private JPanel createOrderRow(PedidoH order) {
    ...
    private void viewOrder(PedidoH order) {
...
class PedidoH{
    ...
    public PedidoH(int id, String cliente, String adress, String date, String timeR, String timeE, BigDecimal total, String pay, String comment) {
        this.id = id;
